...
export default function LoginPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(submitError.message || "Ошибка входа");
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8 sm:p-10">
          ...
          <h1 className="mb-2 text-center text-3xl font-medium text-[var(--foreground)]">Войти в аккаунт</h1>
          <p className="mb-8 text-center text-[var(--muted-foreground)]">Продолжите подготовку к экзамену</p>
          ...
          <div className="mt-8 text-center">
            <span className="text-[var(--muted-foreground)]">Нет аккаунта? </span>
